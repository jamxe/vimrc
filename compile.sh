...
fix_nvim_appimage() {
    \$SUDO mv /usr/\$1/nvim /usr/\$1/.nvim.appimage.noextract
    echo 'x=\$\$; mkdir -p /tmp/_nvim_appimg_.\$x && bash -c \"cd /tmp/_nvim_appimg_.\$x && /usr/\$1/.nvim.appimage.noextract --appimage-extract > /dev/null 2>&1\" && /tmp/_nvim_appimg_.\$x/squashfs-root/AppRun \"\$@\"; x=\$?; rm -rf /tmp/_nvim_appimg_.\$x exit \$x' | \$SUDO tee /usr/\$1/nvim
    \$SUDO chmod +x /usr/\$1/nvim
}
install_nvim() {
    ...
    if [ \"x\$(uname -sm)\" = \"xLinux x86_64\" ]; then
        ...
        (\$SUDO cp ./nvim.appimage /usr/bin/nvim && (/usr/bin/nvim --version || fix_nvim_appimage bin)) \|\| (\$SUDO cp ./nvim.appimage /usr/local/bin/nvim && (/usr/local/bin/nvim --version || fix_nvim_appimage local/bin))
